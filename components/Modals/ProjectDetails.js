...
const ProjectDetails = ({modalShow,setModalShow,modalData}) => {
    return (
       <div className={`duration-100 ${modalShow?'':'!w-0 !h-0 -z-50 opacity-0 transition-all absolute'}`}>
         <div className={`fixed top-0 duration-300 left-0 w-screen h-screen  z-50 flex justify-center items-center`}>
            {/* Outside of modal */}
        <div onClick={()=>setModalShow(!modalShow)} className='backdrop-blur-md fixed w-screen h-screen top-0 left-0 '></div>

        {/* Modal body */}
         <div className="max-w-2xl overflow-hidden bg-base-200 z-50 rounded-lg shadow-md dark:bg-gray-800 ">
    ...
    <div className="p-6">
        <div>
            ...
            <a href="#" className="block mt-2 text-xl font-semibold text-gray-800 transition-colors duration-300 transform dark:text-white hover:text-gray-600 hover:underline" tabindex="0" role="link">{modalData.data}</a>
